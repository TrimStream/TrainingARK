...
export function PlayerZone({ playerIndex, position, revealAll }: PlayerZoneProps) {
	...
	// Floating add card button position
		...
		<div className={styles.playmat}>
			...
			{showCardSearch && cardSearchRect && (
				<div style={{ position: 'fixed', top: cardSearchRect.bottom + 4, left: cardSearchRect.left, zIndex: 1000 }}>
					{/* Inline search -- reuse pattern from CardSearchPopover */}
					<CardAddSearch
						decklist={decklist}
...
// Inline card search for adding cards to zones
// Separate from CardContextMenu -- this searches Scryfall and respects decklist
function CardAddSearch({ decklist, onSelect, onClose }: {
	...
	function filterByDecklist(names: string[]): string[] {
		if (decklist.length === 0) return names
		const q = query.toLowerCase()
		// Return decklist cards that contain the query string
	...
	useEffect(() => {
		...
		debounceRef.current = setTimeout(async () => {
			...
			try {
				...
				setSuggestions(filterByDecklist((data.data ?? []).slice(0, 8)))
				setActiveIndex(-1)
			} finally {
				setLoading(false)
			}
