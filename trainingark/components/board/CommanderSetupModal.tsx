...
export function CommanderSetupModal({ playerIndex, playerName, onConfirm }: CommanderSetupModalProps) {
  ...
  useEffect(() => {
    ...
    if (commanderQuery.length < 2) { setCommanderSuggestions([]); setActiveIndex(-1); return }
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(`https://api.scryfall.com/cards/autocomplete?q=${encodeURIComponent(commanderQuery)}`)
        if (!res.ok) return
        const data = await res.json()
  ...
  function handleConfirm() {
    ...
    const libraryCount = 100 - commanderNames.length - 0 // cards placed in zones later
